...
### 3. 验证证据（Evidence - 实测结果与日志支撑）
```text
修复前实测：时基偏差11.4%导致90度指令实际偏15.4度、6个测试关节中4个被静默截断(如elbow -96.4度被压成0度)、越界角度静默夹紧无告警。修复后：时基偏差0.000%脉宽误差降至0.27~0.45度、elbow -96.4度正确映射为舵机83.6度、越界角度显式拒绝、整臂含越界关节时整体拒绝下发；关节限位使可达域从80.8%收窄至55.4%(排除舵机转不到的姿态)，运动学回环误差保持0.126mm。
```
