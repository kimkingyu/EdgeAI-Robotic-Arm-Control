...
板端没装 perf，`apt` 安装又需要交互式 sudo 密码，于是用 `perf_event_open` 直接读 ARMv8 PMU（`paranoid=2` 允许测自身进程，不需要 root），实现在 [compiler/tests/pmu_counters.c](compiler/tests/pmu_counters.c)。

首轮一次打开 12 个事件，缩放比只有 0.38–0.45——6 个硬件槽位被复用，`branch_instructions` 直接读不到。改为分 4 组依次打开后缩放比全部为 1.0，下表只用这批干净数据：

| 指标 | OpenCV | 融合＋目标复用 | 分块 4×32＋向量化 |
|---|---|---|---|
| 每输出字节指令数 | **11.59** | **82.38** | 83.44 |
| 每输出字节周期数 | 6.49 | 35.37 | 36.01 |
| IPC | 1.79 | **2.33** | 2.32 |
| 后端停顿占比 | **0.41** | 0.20 | 0.16 |
| L1D 读缺失率 | 0.0052 | **0.0042** | 0.0025 |
| LL 读缺失率 | 0.443 | 0.714 | 0.669 |
| L1I 重填率 | 0.0007 | 0.0001 | 0.2365 |

**结论是指令数，不是访存。** 判据：

- MLIR 的 IPC（2.33）**高于** OpenCV（1.79）——若受访存阻塞，IPC 应当更低；
- MLIR 后端停顿占比 0.20，**低于** OpenCV 的 0.41；
- MLIR 的 L1D 读缺失率也更低；
- LL 读缺失次数只差 1.29 倍，远小于周期数的 5.45 倍差距；
- L1I 重填率 0.0001–0.0007，取指不是瓶颈。

也就是说，生成代码的**执行效率并不差，是要执行的工作量多了 7.1 倍**。OpenCV 的 `INTER_LINEAR` 是长期手工优化的 NEON 实现（每字节 11.6 条指令），自建内核每字节 82.4 条，差距在代码生成质量而非调度策略。

这也解释了为什么分块和向量化都没能翻盘——它们改的是访存局部性与并行度，而瓶颈在指令总量；`tile_4x32_vector` 的指令数甚至比未分块版本还略高。

一点如实说明：`branch_instructions` 事件不可用，改用原始事件 0x0D 虽打开成功但恒读 0（该 PMU 未实现），所以**分支开销既未量化也不能声称已排除**。

证据：[PMU 归因汇总](docs/benchmarks/mlir_pmu_attribution.json)。

### 那 7.1 倍指令花在哪：反汇编对照

接着把生成代码逐段拆开看。最内层循环 `.LBB0_7` 共 **74 条指令，每次只产出 1 个输出字节**（与 PMU 实测 82.4 条/字节吻合，差值是外层循环摊销）。按助记符做互斥分类，合计正好 74：
...
| 比较（`cmp`） | 9 |
| 条件选择（`csel`） | 9 |
| 定点除法（`smulh`/`asr`/符号位 `lsr`） | 5 |
| clamp 下界（`bic` 负数归零） | 4 |
| 条件比较（`ccmp`） | 4 |
| 源像素加载（`ldrb`，双线性 4 点） | 4 |
| 位移拼接（`lsl`/`orr`） | 4 |
| 输出写入（`strb`） | 1 |

**来源一：坐标与权重按字节重算。** 融合后的 `linalg.generic` 是三维全 `parallel`（行/列/通道），整套源坐标计算留在最内层 region，对每个 `(行,列,通道)` 组合都执行一次。但这些计算与通道无关、x 方向每行相同：

- x 方向坐标＋权重：只需 640 次，实际 1228800 次——**冗余 1920 倍**
- y 方向同理 1920 倍；通道维再重复 3 倍

坐标与 clamp 相关指令（除法 5＋`bic` 4＋`csel` 9＋`ccmp` 4＋`cmp` 9）占内层 **41.9%**。但即便完全提到循环外，估算也只降到 47.9 条/字节，**仍比 OpenCV 慢 4.13 倍**——所以这只解释了一部分。

**来源二：向量加载退化为逐通道插入。** 向量化版本的 **1536 条 `ld1` 全部是单字节插入形式**（`ld1 {v0.b}[i]`），没有一条连续向量加载。这是 `vector.gather` 的代价：每个源像素单独发一条加载再插进通道，凑满一个 128 位向量要 16 条 `ld1`，而 OpenCV 一条 `ld1 {v0.16b}` 就够。SIMD 只用在乘加上（384 条 `mla`），**加载侧完全没受益**——这也解释了为什么 `tile_4x32_vector` 的指令数（83.4）反而略高于未分块版（82.4）。

**参照系也要说清**：板端 OpenCV 4.11.0 以 **NEON 为 baseline** 编译（另有 `NEON_DOTPROD`/`NEON_FP16` 运行时分发），对比对象是长期手工优化的向量化实现，不是朴素标量代码。每字节 11.6 条指令是这个量级的结果。

要真正追上需要两件事，都不是参数调优而是重新设计算子表达方式：让源地址计算走 affine 可分析形式以便提出循环外；让向量化产生连续 `ld1` 而非 gather。收益与工作量需另行评估。

证据：[指令级归因](docs/benchmarks/mlir_instruction_attribution.json)。
...
> 这里的绝对耗时与比值都受 CPU 降频影响，见后文「测量条件核查」：NPU 推理期间 `ondemand` 会降频，把比值从纯 CPU 条件下的 4.38× 压缩到 3.02×。要看预处理内核本身的相对性能，用 M4 的数字。
...
横向比较依赖"绑定核没有降频"。边界快照（首尾各 1 次）显示所有阶段都恒定在 2.4 GHz，看起来没问题。但边界快照证明不了过程，于是用 [compiler/tests/monitor_frequency.py](compiler/tests/monitor_frequency.py) 以 50 ms 间隔连续采样重跑，结论变了：

| 阶段 | 负载构成 | 采样数 | 低于满频占比 |
|---|---|---|---|
| M4 消融 640×640 | 纯 CPU 预处理 | 1156 | 0.087%（仅第 0 点，基准未开始） |
| M6 A/B yolo640 | 预处理 9.5 ms + NPU 推理 38 ms | 308 | **69.8%** |

M6 的频率在 408 MHz–2.4 GHz 全档位剧烈跳动。原因是 NPU 推理期间 CPU 在等待，`ondemand` 随即降频，而预处理的计时窗口会落在低频段上。

后果可以直接量化——同一个 `fused_destination` 内核、同一份输入：

| | M4（全程满频） | M6（69.8% 低于满频） | 劣化 |
|---|---|---|---|
| OpenCV 预处理 P50 | 4359.2 us | 9595.9 us | 2.20× |
| MLIR 预处理 P50 | 19103.1 us | 29009.8 us | 1.52× |
| **比值** | **4.38×** | **3.02×** | — |

两者受降频影响程度不同（OpenCV 2.20× vs MLIR 1.52×），比值因此从 4.38 被**压缩**到 3.02。

**所以 M6 原先报的 3.02× 不是更乐观的真实结果，而是降频污染的产物。干净条件下的数字是 M4 的 4.38×。** 前文 M6 表格里的 3.04× 同理，只能用于"该配置在混合负载下的实测表现"，不能当作预处理内核的相对性能。

这不改变总体结论方向——MLIR 在任何条件下都更慢，只是差距比污染后的数字更大。`soc-thermal` 降频点 75 °C、实测最高 60.1 °C，所以波动来自 `ondemand` 的负载响应而非过热。测量全程只读 sysfs，没有写 governor。
