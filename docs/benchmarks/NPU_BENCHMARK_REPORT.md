...
> ⚠️ **重要更正（2026-09-17 补记）**：本报告的压缩率、算子直通率、时延与吞吐数据本身
> 都准确，但后来实现后处理解码时发现 —— **本报告全部测试所用的 INT8 模型，其分类分支
> 在量化后恒为 0，检测功能实际不可用**。当时只核对了体积与算子直通率，没有验证输出数值。
>
> 根因是数学必然：YOLOv8 输出头把坐标（0~640）与类别分数（0~1）拼在同一个 84 通道张量，
> 共享量化参数时 scale≈2.51，单个量化步长已大于分数最大值 0.93。`normal`/`mmse`/
> `kl_divergence` 三种算法实测全部如此。
>
> **因此本报告应被理解为「推理速度与算子调度的实测」，而非「检测可用性的验证」。**
> 文中所有 FPS 数字都是模型在输出全零时跑出的 —— 速度是真的，但不可用的速度不构成成果。
> 当前已切换到 FP16（单实例 10.1 FPS，慢 2.3 倍，分类正常）。
> 根因分析见 [`yolo_int8_classification_failure.json`](yolo_int8_classification_failure.json)。
